...
class ADMMUpdater(val lambda: Double) extends Serializable {
     ...
     */
    def wUpdate(partitionData: Iterator[(Double, BDV[Double])],
        ...
        val costFunction = new DiffFunction[BDV[Double]] {
            def calculate(w: BDV[Double]) = {
        ...
        val lbfgs = new LBFGS[BDV[Double]](maxIter = 100, m = 7, tolerance = 1e-5)
     ...
     */
    def zUpdate(w_avg: BDV[Double], u_avg: BDV[Double], rho: Double, numPartitions: Long): BDV[Double] = {
     ...
     */
    def uUpdate(currentState: ADMMState, z_global: BDV[Double]): ADMMState = {
...
class ADMMOptimizer(val lambda: Double, val numPartitions: Int, val n_features: Int) {
    ...
    def run(data: RDD[(Double, BDV[Double])], maxIterations: Int): BDV[Double] = {
        ...
        var states = partitionedData.mapPartitions(_ => Iterator(ADMMState(
            w = BDV.zeros[Double](n_features),
            u = BDV.zeros[Double](n_features)
        )), preservesPartitioning = true).persist(StorageLevel.MEMORY_AND_DISK)

        var z = BDV.zeros[Double](n_features)
        var z_prev = BDV.zeros[Double](n_features)
        var rho = 1.0
        ...
        while (!converged && iter < maxIterations) {
            ...
            val (w_sum, u_sum) = states_after_w.map(s => (s.w, s.u)).treeAggregate(
                (BDV.zeros[Double](n_features), BDV.zeros[Double](n_features))
            )(
...
object ADMMRunner {
    def main(args: Array[String]): Unit = {
        ...
        val data = MLUtils.loadLibSVMFile(sc, dataPath).map {
            lp => (lp.label, BDV(lp.features.toArray))
        }
