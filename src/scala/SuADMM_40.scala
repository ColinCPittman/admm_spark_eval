 ...
 */
class SquaredL2Updater_40(val regularizationParam_lambda: Double) extends ADMMUpdater_40 {
...
class ADMMOptimizer_40(
    ...
    def run(data: RDD[(Double, DenseVector[Double])], maxIterations: Int): (DenseVector[Double], Int) = {
        ...
        var statesRDD = partitionedData.mapPartitions(_ => Iterator(ADMMState_40(
 ...
 */
abstract class GeneralizedLinearAlgorithm_40
class LogisticRegressionWithADMM_40(
    val regularizationParam_lambda: Double,
    val numPartitions: Int,
    val maxIterations: Int
) extends GeneralizedLinearAlgorithm_40 {
    ...
    def train(data: RDD[(Double, DenseVector[Double])]): (DenseVector[Double], Int) = {
        ...
        val updater = new SquaredL2Updater_40(regularizationParam_lambda)
        val optimizer = new ADMMOptimizer_40(updater, numPartitions, numFeatures)
...
object ADMMRunner_40 {
    def main(args: Array[String]): Unit = {
        if (args.length < 4) {
            System.err.println("Usage: ADMMRunner_40 <data_path> <num_partitions> <num_features> <output_path>")
            System.exit(1)
        }
        ...
        val admmLogisticRegression = new LogisticRegressionWithADMM_40(
 ...
 */
def calculateAccuracy_40(testData: RDD[(Double, DenseVector[Double])], model: DenseVector[Double]): Double = {
 ...
 */
def splitTrainTest_40(data: RDD[(Double, DenseVector[Double])], trainRatio: Double = 0.8): (RDD[(Double, DenseVector[Double])], RDD[(Double, DenseVector[Double])]) = {
...
/**
 * Main ADMM function with train/test split and accuracy evaluation.
 * Supports dataset parameter for easy switching between RCV1 and HIGGS.
 */
def runADMM(
    dataset: String = "rcv1",
  ...
  val learner = new LogisticRegressionWithADMM_40(lambda, numPartitions, maxIterations)
  ...
  val accuracy = calculateAccuracy_40(testData, model) * 100.0
  ...
  // Smart filename generation with version, partitions, and run tracking
  val sparkVersion = "4_0"  // Spark 4.0 version
  
  // Extract sample counts from filenames (e.g., "rcv1_train15k" -> "15k")
...
def runLBFGS(
  ...
  // convert labels {-1,1} -> {0,1} for MLlib
  val trainData = loadDataSmart(sc, trainPath, dataset.toLowerCase == "higgs")
  ...
  // create MLlib LogisticRegressionWithLBFGS
  val lbfgs = new org.apache.spark.mllib.classification.LogisticRegressionWithLBFGS()
    .setNumClasses(2)
    .setIntercept(true)
  
  // set hyperparameters to match paper setup
  lbfgs.optimizer
  ...
  // evaluate accuracy on test set using MLlib evaluation
  ...
  // extract model weights for analysis
